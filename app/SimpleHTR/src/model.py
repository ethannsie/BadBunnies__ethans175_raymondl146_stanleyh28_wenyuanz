...
class Model:
    ...
    def setup_cnn(self) -> None:
        ...
        pool = self.input_imgs  # already 4D input shape: (batch, height, width, channels)

        for i in range(num_layers):
            kernel = tf.Variable(
                tf.random.truncated_normal(
                    [kernel_vals[i], kernel_vals[i], feature_vals[i], feature_vals[i + 1]], stddev=0.1
                )
            )
            conv = tf.nn.conv2d(input=pool, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))

            # Use BatchNormalization with training arg
            bn = BatchNormalization()
            conv_norm = bn(conv, training=self.is_train)
